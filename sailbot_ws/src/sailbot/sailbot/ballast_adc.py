...
class BallastADC(Node):
    def __init__(self):
        ...
        self.bus0 = smbus.SMBus(0)  # use 0 for older versions of Jetson Nano
        self.bus1 = smbus.SMBus(1)  # use 0 for older versions of Jetson Nano
    ...
    def read_raw_data(self, addr):
        ...
        high = self.bus1.read_byte_data(self.Device_Address, addr)
        low = self.bus1.read_byte_data(self.Device_Address, addr + 1)
    ...
    def Magnetometer_Init(self):
        ...
        self.bus1.write_byte_data(self.Device_Address, self.Register_A, 0x70)

        # Write to Configuration Register B for gain
        self.bus1.write_byte_data(self.Device_Address, self.Register_B, 0xa0)

        # Write to mode Register for selecting mode
        self.bus1.write_byte_data(self.Device_Address, self.Register_mode, 0)
    ...
    def calcADC(self):
        config = [0x40, 0x83]
        self.bus0.write_i2c_block_data(self.address, 0x01, config)
        time.sleep(0.1)
        # read the data from the ADC
        data = self.bus0.read_i2c_block_data(self.address, 0x00)
        ...
        self.bus0.write_i2c_block_data(self.address, 0x01, config)  # tell the ADC to read off of the other pin
        time.sleep(0.1)
        data = self.bus0.read_i2c_block_data(self.address, 0x00)
